...
更新不再每次强制重建所有容器。镜像拉取后固定准确镜像 ID，由 Compose 根据镜像和配置变化决定是否重建；若本次修正了 `.env`，仍需重建 Xboard 刷新文件挂载。配置无需修正时保留原文件。仍会执行迁移、缓存刷新和必要重启，补完可能中断的更新步骤。日志中的“镜像未变化”不等于跳过所有更新步骤。

安装、更新和修复的 Redis 就绪等待默认最长 600 秒，每约 30 秒显示进度。NAS 重建容器后可能先执行目录权限处理，等待期间不要重复点击更新。检查以 Redis 返回 `PONG` 为准，容器退出会提前报错；超过上限仍会停止，不会谎报成功或重建数据库。可在 `deploy.env` 设置 `XBOARD_REDIS_WAIT_SECONDS=900`（范围 1-3600）适当延长；长期 I/O 阻塞仍需排查存储。
...
镜像也在备份中，因此包会比旧版本大。预检会为镜像、文件和卷快照预留磁盘空间，并使用快速压缩减少 CPU 开销。打包期间服务会暂停；成功后默认重新启动原容器，备份或重新启动失败会明确返回失败。旧备份和历史恢复目录不会自动清理。
...
卸载只处理能核对到当前 Compose 工作目录的容器和数据卷，不按名称前缀删除全机资源。镜像缓存全部保留，避免影响其他项目；外部/共享卷、未选择删除的备份及历史恢复目录也会保留并提示。自定义备份目录还需要有效的归属标记；符号链接、危险路径和混放其他文件的备份目录会在停止容器前被拒绝清理。
...
健康检查验证本机容器、应用身份下的数据库和 Redis、管理员、后台 HTML、公开配置 JSON 和 NPM 后端健康接口。短暂启动阶段会提示等待就绪，最终仍未通过才输出完整错误。仅有端口监听或根路径返回 4xx 不代表通过；本机检查不代替公网 DNS、防火墙、证书和完整业务测试。
